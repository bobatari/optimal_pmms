...
function getMediaErrorMessage(media, fallback) {
	if (media && media.error && media.error.message) {
		return media.error.message;
	}

	if (media && typeof media.generateError === 'function' && media.currentSrc) {
		return 'Unknown media play error';
	}

	return fallback || 'Unknown media error';
}
...
function resolveUrl(url) {
	if (url.startsWith('http://') || url.startsWith('https://')) {
		...
		return 'http://' + currentServerEndpoint + '/' + resourceName + '/media/' + url;
	}
}
	...
	new MediaElement(id, {
		error: function(media) {
			...
			sendMessage('initError', {
				url: options.url,
				message: getMediaErrorMessage(media, 'Failed to initialize media')
			});
		...
		success: function(media, domNode) {
			...
			media.addEventListener('error', event => {
				...
				sendMessage('playError', {
					url: options.url,
					message: getMediaErrorMessage(media, 'Failed to play media')
				});
...
function updateUi(data) {
	...
	if (data.uiIsOpen) {
		...
		var selectedHandle = data.selectedHandle;
		var usableMediaPlayersValue = selectedHandle || usableMediaPlayersSelect.value;
		...
		if (presetKeys.length > 0) {
			...
			presetSelect.style.visibility = 'visible';
		} else {
...
window.addEventListener('load', () => {
	sendMessage('startup', {}).then(resp => resp.json()).then(resp => {
		...
		defaultVideoSize = resp.defaultVideoSize || defaultVideoSize;
		...
		keys.sort((a, b) => {
			return audioVisualizations[a].name.localeCompare(audioVisualizations[b].name);
		});
		keys.forEach(key => {
			var option = document.createElement('option');
			option.value = key;
