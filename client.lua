...
local function openControlsUiForHandle(handle)
	selectedUiHandle = handle
	SendNUIMessage({
		type = "showUi"
	})
...
local function updateUi()
	...
	SendNUIMessage({
		...
		selectedHandle = selectedUiHandle
	})

	if selectedUiHandle then
...
AddEventHandler("pmms:showControls", function()
	openControlsUiForHandle(selectedUiHandle)
end)
...
RegisterNetEvent("optimal_pmms:client:openTargetMediaPlayer", function(entity)
	if not doesEntityExist(entity) then
		return
	end

	if not Config.models[GetEntityModel(entity)] then
		return
	end

	enableEntity(entity)
	openControlsUiForHandle(getHandle(entity))
	updateUi()
end)

local function registerOxTarget()
	if oxTargetRegistered then
		return
	end

	if not Config.enableOxTargetSelection then
		return
	end
	...
	local targetOptions = {
		{
			name = "optimal_pmms_open_controls",
			icon = "fa-solid fa-tv",
			label = "Open Media Controls",
			distance = Config.oxTargetSelectionDistance,
			canInteract = function(entity)
				if not doesEntityExist(entity) then
					return false
				end

				if not permissions.interact then
					return false
				end

				return Config.models[GetEntityModel(entity)] ~= nil
			end,
			onSelect = function(data)
				TriggerEvent("optimal_pmms:client:openTargetMediaPlayer", data.entity)
			end
		}
	}

	exports.ox_target:addGlobalObject(targetOptions)
	exports.ox_target:addGlobalVehicle(targetOptions)

	oxTargetRegistered = true
end
